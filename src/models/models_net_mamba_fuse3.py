...
class NetMamba(nn.Module):
    def __init__(self, 
        ...
        self.encoder_blocks = nn.ModuleList([
            create_block(embed_dim, norm_epsilon=1e-5, rms_norm=True, fused_add_norm=True, 
                        layer_idx=i, bimamba_type=bimamba_type, drop_path=inter_dpr[i],
                        if_devide_out=True, residual_in_fp32=True) for i in range(encoder_depth)])
        ...
        if is_pretrain:
            ...
            self.byte_decoder_blocks = nn.ModuleList([
                create_block(decoder_embed_dim, norm_epsilon=1e-5, rms_norm=True, fused_add_norm=True, 
                            layer_idx=i, bimamba_type=bimamba_type, drop_path=decoder_inter_dpr[i],
                            if_devide_out=True, residual_in_fp32=True) for i in range(decoder_depth)])
    ...
    def forward_encoder(self, x_byte: torch.Tensor, x_size: torch.Tensor, x_iat: torch.Tensor,
        ...
        x_byte = self.byte_embed(x_byte.reshape(B, C, -1)) # [B, Lb, D]
        ...
        x_size = self.size_embed(x_size)  # [B, Ls, D]
        ...
        x_iat = self.iat_embed(x_iat)  # [B, Ls, D], use size embed for IAT
    ...
    def forward(self, x_byte: torch.Tensor, x_size: torch.Tensor, x_iat: torch.Tensor,
        ...
        if self.is_pretrain:
            ...
            latent, byte_mask, ids_restore, size_mask, iat_mask = self.forward_encoder(
                x_byte, x_size, x_iat,
                byte_mask_ratio=byte_mask_ratio, size_mask_ratio=size_mask_ratio, iat_mask_ratio=iat_mask_ratio,
            ...
            latent = self.forward_encoder(x_byte, x_size, x_iat, byte_mask_ratio=0.0, size_mask_ratio=0.0, iat_mask_ratio=0.0, if_mask=False)
...
def fuse3_mamba_pretrain(**kwargs):
    model = NetMamba(
        is_pretrain=True, embed_dim=256, encoder_depth=4, decoder_embed_dim=128, decoder_depth=2, 
        **kwargs)
    return model

def fuse3_mamba_classifier(**kwargs):
    model = NetMamba(
        is_pretrain=False, embed_dim=256, encoder_depth=4, **kwargs)
    return model
